...
if SERVER then
    ...
    ---
    -- @realm server
    cvCrowbarUnlocks = CreateConVar("ttt_crowbar_unlocks", "0", { FCVAR_ARCHIVE, FCVAR_NOTIFY })
...
hook.Add("TTTPlayerSpeedModifier", "CrowSpeed" , function(ply, _, _, noLag )
    local wep=ply:GetActiveWeapon()
    if wep and IsValid(wep) and wep:GetClass()=="weapon_zm_improvised" then
      if TTT2 then
        noLag[1] = noLag[1] * 1.3
      else
        return 1.3
