...
if CLIENT then
   ...
   SWEP.EquipMenuData = {
      type = "item_weapon",
      desc = "Kills targets instantly from behind.\n Throw for an instant kill."
   };
...
function SWEP:PrimaryAttack()
   ...
   if SERVER and tr.Hit and tr.HitNonWorld and IsValid(hitEnt) then
      if hitEnt:IsPlayer() then
         ...
         if ((hitEnt:Health() < (self.Primary.Damage + 10)) or (math.abs(attack_angle) < 90) or (math.abs(attack_angle) > 270)) then
...
hook.Add("TTTPlayerSpeedModifier", "KnifeSpeed" , function(ply, _, _, noLag )
    local wep=ply:GetActiveWeapon()
    if wep and IsValid(wep) and wep:GetClass()=="weapon_ttt_knife" then
      if TTT2 then
        noLag[1] = noLag[1] * 1.3
      else
        return 1.3
